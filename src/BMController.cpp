...
void BMController::locationCallback(const geometry_msgs::Polygon& msg)
{
    ...
    std::cout << id << "'s location: [" << loc.x << ", " << loc.y << ", " << loc.z << "] = [" <<
              gridLoc.x << ", " << gridLoc.y << ", " << gridLoc.z << "]" << std::endl;
...
void BMController::findAlternatePaths()
{
    ...
    dstar.updateStart(self.x, self.y);
    ...
    for (const auto kvp : robotLocations)
    {
        if (kvp.first != myID)
        {
            ...
            dstar.updateCell(other.x, other.y, NONTRAVERSABLE_COST);
        }
    }
    ...
    for (const auto kvp : robotLocations)
    {
        if (kvp.first != myID)
        {
            ...
            {
                dstar.updateCell(neighbor.x, neighbor.y, NONTRAVERSABLE_COST);
            }
        }
    }
    ...
    for (const auto kvp : robotLocations)
    {
        ...
        {
            Point3D other = kvp.second;
            dstar.updateCell(other.x, other.y, DEFAULT_COST);
            for (const auto neighbor : grid->getNeighbors(other))
            {
                dstar.updateCell(neighbor.x, neighbor.y, DEFAULT_COST);
            }
        }
    }
...
void BMController::alternativeCallback(const geometry_msgs::Polygon& msg)
{
    ...
    std::cout << id << "'s" << altNumber << (altNumber == 1 ? "st" : "nd") << "alternative: [" << loc.x << ", " << loc.y << ", " << loc.z << "]\n";
