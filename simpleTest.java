...
//    		return;
    ...
    static boolean testMultiRobot(String robotPrefix, VrepRobotType type)
    {
    	...
    	for (IVrepRobot bot : bots)
    	{
    		Point2D gridPoint = g.getGridCoordinates(bot.getLocation());
    	...
    	for (IVrepRobot bot : bots)
    	{
    		Point2D myGridPos = g.getGridCoordinates(bot.getLocation());
    		// Find most distant robot.
    		double farthestDist = 0;
    		IVrepRobot farthestRobot = bot; 
    		for (IVrepRobot other : bots)
    		{
    			Point2D otherGridPos = g.getGridCoordinates(other.getLocation());
    		...
    		List<Point2D> path = as.findPath(myGridPos, g.getGridCoordinates(farthestRobot.getLocation()));
    		System.out.format("Driving %s...\n", bot.toString());
    		for (Point2D waypoint : path)
    		{
    			bot.driveTo(g.getWorldCoordinates(waypoint), 0.5f);
    		}
    ...
    {
    	Grid g = new Grid(9, 9, 10, 10, new PointF3D(0, 0, 0), 4);
    	AStar astar = new AStar(g);
    	
    	Point2D origin = new Point2D(0, 0);
    	Point2D destination = new Point2D(8, 8);
